...
{
    public class EditCategotyRequest : IRequest<int>
    {
    ...
    class EditCategoryHandler : IRequestHandler<EditCategotyRequest, int>
    {
        private readonly IQuery _query;
        public EditCategoryHandler(IQuery query)
        {
            _query = query;
        }
        public async Task<int> Handle(EditCategotyRequest request, CancellationToken cancellationToken)
        {
            string sql = "updateCategory";
            DynamicParameters parameter = EditCategoryHandler.addAllParameterCategory(request);
            int result = _query.Execute(sql, parameter);
            return await Task.FromResult(result);
        }
        public static DynamicParameters addAllParameterCategory(EditCategotyRequest category)
        {
