...
{
    public class EditPostRequest : IRequest<int>
    {
    ...
    class EditPostHandler : IRequestHandler<EditPostRequest, int>
    {
        private readonly IQuery _query;
        public EditPostHandler(IQuery query)
        {
            _query = query;
        }
        public async Task<int> Handle(EditPostRequest request, CancellationToken cancellationToken)
        {
            string sql = "updatePost";
            request.Slug = Helper.ToSlug(request.Title);
            DynamicParameters parameter = EditPostHandler.addAllParameterPost(request);
            int result = _query.Execute(sql, parameter);
            return await Task.FromResult(result);
        }
        public static DynamicParameters addAllParameterPost(EditPostRequest post)
        {
